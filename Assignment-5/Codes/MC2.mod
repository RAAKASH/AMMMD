...
# Each node which has to be visited daily should be a part of route 1 and route 2
# and only 1 node should be visited from it in each route
Outdegree_Daily_Route1 {i in Nodes : D[i] = 1}: sum{j in Nodes: i<>j} PR1[i,j] = 1;
Outdegree_Daily_Route2 {i in Nodes : D[i] = 1}: sum{j in Nodes: i<>j} PR2[i,j] = 1;

# Each node which has to be visited on alternate days should be a part either route 1 or route 2
# and only 1 node should be visited from it in the respective route
Outdegree_Alternate {i in Nodes : D[i] = 0}: sum{j in Nodes: i<>j} (PR1[i,j] + PR2[i,j]) = 1;

# Indegree = Outdegree
Flux_Route1 {j in Nodes: D[j]=0}: sum{i in Nodes: i<>j} PR1[i,j] = sum{i in Nodes: i<>j} PR1[j,i];
Flux_Route2 {j in Nodes: D[j]=0}: sum{i in Nodes: i<>j} PR2[i,j] = sum{i in Nodes: i<>j} PR2[j,i];
